...
public class RenderEntityHookShot extends Render
{
	...
	public void renderArrow(EntityHookShot hookshot, double x, double y, double z, float yaw, float partialTick) {
		...
		renderer.pos(-7.0D, -2.0D, -2.0D).tex((double)f6, (double)f8);
		renderer.pos(-7.0D, -2.0D, 2.0D).tex((double)f7, (double)f8);
		renderer.pos(-7.0D, 2.0D, 2.0D).tex((double)f7, (double)f9);
		renderer.pos(-7.0D, 2.0D, -2.0D).tex((double)f6, (double)f9);
		tessellator.draw();
		GL11.glNormal3f(-f10, 0.0F, 0.0F);
		renderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
		renderer.pos(-7.0D, 2.0D, -2.0D).tex((double)f6, (double)f8);
		...
		for (int i = 0; i < 4; ++i) {
			...
			renderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
			renderer.pos(-8.0D, -2.0D, 0.0D).tex((double)f2, (double)f4);
			renderer.pos(8.0D, -2.0D, 0.0D).tex((double)f3, (double)f4);
			renderer.pos(8.0D, 2.0D, 0.0D).tex((double)f3, (double)f5);
			renderer.pos(-8.0D, 2.0D, 0.0D).tex( (double)f2, (double)f5);
			tessellator.draw();
		}
